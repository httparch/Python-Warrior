...
;(function (global, factory) {
        ...
        for (var k = 0; k < cm.options.gutters.length; ++k) {
          ...
          if (found) {
            gutterWrap.appendChild(
              elt('div', [found], 'CodeMirror-gutter-elt', 'left: ' + dims.gutterLeft[id] + 'px; width: ' + dims.gutterWidth[id] + 'px')
            )
          }
        }
      }
    ...
    var index = findFirst(
      function (i) {
        var part = order[i],
          ltr = part.level != 1
        return boxIsAfter(
          cursorCoords(cm, Pos(lineNo$$1, ltr ? part.to : part.from, ltr ? 'before' : 'after'), 'line', lineObj, preparedMeasure),
          x,
          y,
          true
        )
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    setValue: docMethodOp(function (code) {
      ...
      makeChange(
        this,
        { from: top, to: Pos(last, getLine(this, last).text.length), text: this.splitLines(code), origin: 'setValue', full: true },
        true
      )
      if (this.cm) {
  ...
  function moveVisually(cm, line, start, dir) {
    ...
    if (cm.doc.direction == 'rtl' || part.level == 1) {
      ...
      if (
        ch != null &&
        (!moveInStorageOrder ? ch >= part.from && ch >= wrappedLineExtent.begin : ch <= part.to && ch <= wrappedLineExtent.end)
      ) {
